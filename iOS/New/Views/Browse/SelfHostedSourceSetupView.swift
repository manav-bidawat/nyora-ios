...
/// Setup for the hosted Nyora content server. Unlike Komga/Kavita this needs no
/// login — just a server URL (the parser API is unauthenticated). We ping
/// `/health`, then register the source.
struct NyoraSetupView: View {
    @Environment(\.presentationMode) private var presentationMode

    @State private var server = "https://api.hasanraza.tech"
    ...
    var body: some View {
        Form {
            Section {
                TextField("https://api.hasanraza.tech", text: $server)
                    .autocapitalization(.none)
                    .disableAutocorrection(true)
                    .keyboardType(.URL)
            } header: {
                Text(NSLocalizedString("SERVER"))
            } footer: {
                Text(NSLocalizedString("The hosted Nyora content server. The default works out of the box."))
            }
            ...
            Section {
                Button {
                    ...
                    HStack {
                        Text(NSLocalizedString("CONNECT"))
                        if connecting {
                            Spacer()
                            ProgressView()
                        }
                ...
                .disabled(connecting || normalizedURL == nil)
            }
        }
    ...
    private func connect() async {
        ...
        // Reachability check against the helper's /health endpoint.
        let healthURL = url.appendingPathComponent("health")
        do {
            let (_, response) = try await URLSession.shared.data(from: healthURL)
            guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
                errorText = NSLocalizedString("Could not reach the server.")
                return
            }
        } catch {
        ...
        await SourceManager.shared.createCustomSource(kind: .nyora, name: "Nyora", server: url)
